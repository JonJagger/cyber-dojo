...
class ChooseTests < AppLibTestBase
  ...
  test 'when no id then choose random known language' do
  ...
  test 'when id is given and !katas[id].exists then choose random known language' do
    kata = dojo.katas['123456789A']
    assert !kata.exists?
    id = '012345'
    assert_is_randomly_chosen_language(test_languages_names, id, katas)
  ...
  test 'when id and katas[id].exists? but language is unknown' +
...
=end
  ...
  test 'when no id then choose random known exercise' do
    kata = dojo.katas['123456789A']
    assert !kata.exists?
    id = nil
  ...
  test 'when id but !katas[id].exist? then choose random known exercise' do
    kata = dojo.katas['123456789A']
    assert !kata.exists?
    id = '123456'
    assert_is_randomly_chosen_exercise(test_exercises_names, id, katas)
  ...
  test 'when id and katas[id].exists? but exercise is unknown then choose random known exercise' do
  ...
  test 'when id and katas[id].exists? and exercise is known then choose that exercise' do
